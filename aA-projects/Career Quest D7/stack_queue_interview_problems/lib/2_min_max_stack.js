...
class MinMaxStack {
    ...
    push(val) {
        ...
        // check if the newnode's value is less than the value of the last element in this.minValues 
        if (!this.minValues.length || newNode.value <= this.minValues[this.minValues.length - 1].value) {
            this.minValues.push(newNode)
        }

        // check if the newnode's value is greater than the value of the last element in this.mmaxValues 
